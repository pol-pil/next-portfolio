...
export default function Dock({
   ...
   return (
      <motion.div
         ...
         transition={spring}
         className={`${className} fixed bottom-2 left-1/2 -translate-x-1/2 flex items-end w-fit gap-4 rounded-2xl border-neutral-700 border-2 pb-2 px-4`}
      ...
      >
         <AnimatePresence initial={false}>
            {items.map((item) => (
               <motion.div
                  key={typeof item.label === 'string' ? item.label : undefined}
                  layout
                  initial={{ opacity: 0, scale: 0.4, width: 0, marginLeft: 0, marginRight: 0 }}
                  animate={{ opacity: 1, scale: 1, width: 'auto', marginLeft: 0, marginRight: 0 }}
